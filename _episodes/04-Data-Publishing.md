...
Prior to computers and the internet, pubishing data routinely was really not possible beyond what we could publish in books.  Consequently, a culture grew up around scholarly publishing where data were considered disposable after some specified regulatory period, and certainly where the production of a data set on its own was not considered a work of scholarship.  Rather it was the hypotheses proposed, the experimental design, the analysis and the insights gained from collected data that were valued and preserved through our system of journals and books.  
 
Few would argue that the full potential of a data set can be captured in prose, but the scientific article has held such a privileged place in scientific communication for so long that the current push to treat scientific data differently than in the past has encountered much resistance across fields. Some fields have come together to require publishing primary data, e.g., genomics, protein structure (ref). But the resistance has been much greater in fields such as neuroscience characterized by a multiplicity of data types and relatively small data sets produced by small teams of scientists through often complex experimental designs, so called “long tail” data (Ferguson, Nielson, Cragin, Bandrowski, & Martone, 2014)
 
Almost all major funding agencies in the US and abroad are developing policies around open sharing of research data and other research products (Gewin, 2016; McKiernan et al., 2016).  The impetus for the push towards open science, which at a minimum encompasses the open sharing of the products of research (such as research articles, data and code) is driven by both societal and technological factors. Societal concerns focus on the integrity of scientific research, which calls for greater transparency in light of recent concerns about scientific reproducibility across several fields (Gewin, 2016; Open Science Collaboration et al., 2015) and the rights of the public to access research results generated from taxpayer funding. Technological factors reflect both the need to modernize our scholarly communications system in light of new, disruptive technologies (the internet, mobile devices) and the promises of new insights to be gained from increased human- and machine-based access to research outputs. 
 
Towards these ends, we are seeing more and more calls for domains not just to establish a basic culture of human-centric sharing, i.e., “upon request”, but to move towards a more e-Science vision, where researchers conduct their research digitally and where data standards and programmatic interfaces make it easy for machines to access and ingest large amounts of data.  It requires that the products of research, including the data, be easily coupled to computational resources that can operate on them at scale, and that the provenance of these research products can be tracked as they transition between uses. It requires the ability to find, access and reuse digital artifacts using computational interfaces (API’s) with minimal restrictions. 
...
**3) Plan ahead when publishing your data:**  The process and costs associated with publishing research articles are well known to researchers, and they ensure that they include adequate resources within their research proposals to ensure that they can prepare and publish articles.  Data are a lot more varied in size and complexity than research articles, and thought must be given to how they are going to be published before they are collected. For data of a reasonable size, most repositories are still hosting the data free of charge. Some repositories, e.g., NDAR, require a fee to deposit data, but they also provide cost estimates that can be included within grant proposals.  The new NIH Data Commons pilot is recognizing that NIH supported researchers may not always adequately budget for storage and curation costs, and so is piloting a "Commons credit project" (add link). 

In general, it is best to consult with a data repository regarding your plans for publishing data to ensure that the data are collected according to the appropriate standards and that costs are covered.

In addition to costs, just as with publishing articles, you need to ensure that all parties who have contributed to the data are credited and agree to publishing the data (see below: Data citation).  
